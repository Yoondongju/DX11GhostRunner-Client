...
void CBody_Player::Late_Update(_float fTimeDelta)
{
	...
	m_pGameInstance->CollisionUpdate_PlayerToTriangleMeshGeometry(m_vDir,m_vDepth,m_pShape, &m_PxTransform);

	_float4x4* pParentMatrix = const_cast<_float4x4*>(m_pParentMatrix);
	pParentMatrix->m[3][0] = m_PxTransform.p.x;
	pParentMatrix->m[3][1] = m_PxTransform.p.y;
	pParentMatrix->m[3][2] = m_PxTransform.p.z;
...
HRESULT CBody_Player::Ready_PhysX()
{
	...
	m_pShape = m_pGameInstance->Get_Physics()->createShape(PxCapsuleGeometry(2.f, 3.f), *Material);
	if (!m_pShape) {

#include "stdafx.h"
#include "../Public/EnemyMarkerMC.h"
...
CEnemyMarkerMC::CEnemyMarkerMC(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CUIObject{ pDevice, pContext }
{
}

CEnemyMarkerMC::CEnemyMarkerMC(const CEnemyMarkerMC& Prototype)
	: CUIObject{ Prototype }
{
}

HRESULT CEnemyMarkerMC::Initialize_Prototype()
{

	return S_OK;
}

HRESULT CEnemyMarkerMC::Initialize(void* pArg)
{
	ENEMYMARKERMC_DESC* pDesc = static_cast<ENEMYMARKERMC_DESC*>(pArg);
...
void CEnemyMarkerMC::Priority_Update(_float fTimeDelta)
{
...
void CEnemyMarkerMC::Update(_float fTimeDelta)
{
	// 내 시야각을 먼저 체크하고, 몬스터 갯수체크해서, 스킬을 사용못한다라고 폰트 띄우는걸 먼저 ㄱ  //  나미 , 컷올, 마인드컨트롤 썼을때 일단 가정하고

	m_bActivate = m_pOwner->IsMindControlReady();
	if (false == m_bActivate)
	{
		m_isTaskDone = false;
	...
	vOwnerPos = XMVectorSetY(vOwnerPos, vOwnerPos.m128_f32[1] + 60.f);
	...
	if (true == m_pOwner->IsMindControlReady())
	{
		if (false == m_isTaskDone && m_vTargetSize.x > m_fSizeX &&
			m_vTargetSize.y > m_fSizeY)
		{
			m_fSizeX += fTimeDelta * 250.f;
			m_fSizeY += fTimeDelta * 250.f;
		}
		else
		{
			m_isTaskDone = true;
...
void CEnemyMarkerMC::Late_Update(_float fTimeDelta)
{
	if (false == m_bActivate)
		return;
...
HRESULT CEnemyMarkerMC::Ready_Components()
{
	/* FOR.Com_Shader */
	...
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_EnemyMarkerMC"),
...
CEnemyMarkerMC* CEnemyMarkerMC::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEnemyMarkerMC* pInstance = new CEnemyMarkerMC(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CEnemyMarkerMC"));
...
CGameObject* CEnemyMarkerMC::Clone(void* pArg)
{
	CEnemyMarkerMC* pInstance = new CEnemyMarkerMC(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CEnemyMarkerMC"));
		Safe_Release(pInstance);
	}
